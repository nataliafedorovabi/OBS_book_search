...
ФОРМАТ: Начни с "Вот что я нашёл:" и перечисли ключевые концепции из фрагментов.
Если тема связана с вопросом косвенно - просто покажи что есть, без оправданий.
В конце укажи источник: глава."""
...
ЗАДАЧА: Пойми, что ищет пользователь, и предложи термины для поиска в книге.

Ответь СТРОГО JSON (без пояснений):
{{"search_terms": ["термин1", "термин2", "термин3"]}}

Правила:
- Используй термины из содержания книги
- 2-4 термина, самые релевантные
- Если вопрос про абстрактное понятие - найди связанные концепции из книги"""
